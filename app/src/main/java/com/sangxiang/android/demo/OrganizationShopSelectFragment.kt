...
import kotlinx.android.synthetic.main.fragment_organization_shop_select.view.*
 ...
 *
 */
class OrganizationShopSelectFragment : EmucooBaseFragment(R.layout.fragment_organization_shop_select,false), AnkoLogger {
    ...
    lateinit var mAdapter:BaseQuickAdapter<ContactsResult.AreaItem, BaseViewHolder>
    ...
    override fun initData() {
        arguments?.let {
            multipleSelect=it.getBoolean(OrganizationShopSelectFragment.param_data_multipleSelect,false)
            it.getSerializable(OrganizationShopSelectFragment.param_select_users)?.let {
                list=it as ArrayList<ContactsResult.UserItem>
            }
            contactType=it.getInt(OrganizationShopSelectFragment.param_contact_type,0)
        }

        mAdapter= object :BaseQuickAdapter<ContactsResult.AreaItem, BaseViewHolder>(R.layout.item_organization_shop_select,null) {
            override fun convert(helper: BaseViewHolder, model: ContactsResult.AreaItem) {
