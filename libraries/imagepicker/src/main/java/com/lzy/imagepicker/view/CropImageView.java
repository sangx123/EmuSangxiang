...
//                    bitmap.recycle();
     ...
     * @param isSaveRectangle 是否希望按矩形区域保存图片
     */
    public void saveBitmapToFile(File folder, int expectWidth, final int exceptHeight, boolean isSaveRectangle, final ExifInterface exifInterface) {
        if (mSaving) return;
        mSaving = true;
        ...
        new Thread() {
            @Override
            public void run() {
                saveOutput(croppedImage, finalOutputFormat, finalSaveFile, exifInterface);
            }
        }.start();
    }
...
//        String filename = prefix + dateFormat.format(new Date(System.currentTimeMillis())) + suffix;
        String filename = prefix + System.currentTimeMillis()+(ImagePicker.mRand.nextBoolean()?"a":"b")+ImagePicker.mRand.nextLong() + suffix;
            ...
            if (outputStream != null) {
                ...
                try {
                    if (!TextUtils.isEmpty(date_time)) {
                        ...
                        formatedDateTime = getDateFromPattern(parse.getTime(), "yyyy-MM-dd HH:mm:ss");
