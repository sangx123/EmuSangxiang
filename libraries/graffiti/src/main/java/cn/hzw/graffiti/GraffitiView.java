...
public class GraffitiView extends View {
    ...
    private Bitmap mDelSelectedItemBmp  = null;
    ...
    public GraffitiView(Context context, Bitmap bitmap, String eraser, boolean eraserImageIsResizeable, GraffitiListener listener) {
        ...
        mDelSelectedItemBmp = BitmapFactory.decodeResource(getResources(),R.drawable.icon_graffiti_delete);
//        mDelSelectedItemBmp = Bitmap.createScaledBitmap(mDelSelectedItemBmp,200,200,false);
        mBitmap = bitmap;
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                if (isPenSelectable()) {
                    ...
                    if (mSelectedItem != null) {
                        ...
                        //查看是不是在移除的icon区域
//                        Rect rect = mSelectedItem.getBounds(mGraffitiRotateDegree);
                        float[] xy = mSelectedItem.getXy(mGraffitiRotateDegree);
                        ...
                        Log.e("left???"+left+"  top???"+top+ "  xy(",xy[0]+":"+xy[1] +") touchX~ouchY：（"+mTouchX+":"+mTouchY+ "）   toX~toY：（"+toX(mTouchX)+":"+toY(mTouchY)+")  mSelectedItemX:"+mSelectedItemX+"   mSelectedItemY:"+mSelectedItemY);
                        */
                        ...
                        mDeleteIconRect.left = (int)(xy[0]+bounds.left - mDelSelectedItemBmp.getWidth()/2 );
                        mDeleteIconRect.right = (int)(xy[0] + bounds.left+ mDelSelectedItemBmp.getWidth()/2);
                        mDeleteIconRect.top = (int)(xy[1]+ bounds.top- mDelSelectedItemBmp.getHeight()/2);
                        mDeleteIconRect.bottom = (int)(xy[1]+bounds.top+ mDelSelectedItemBmp.getHeight()/2);
                        Log.e("DELETE:","mDeleteIconRect:"+mDeleteIconRect +"   mSelectedItem.getBound:"+ bounds);

                        if(mDeleteIconRect.contains((int)(toX(mTouchX)),(int)(toY(mTouchY)))){
...
//            }
            ...
            canvas.drawBitmap(mDelSelectedItemBmp,rect.left - mDelSelectedItemBmp.getWidth()/2,rect.top - mDelSelectedItemBmp.getHeight()/2,null);
            Log.e("Real Del Rect:", "left:" + (rect.left - mDelSelectedItemBmp.getWidth()/2)+"   top:"+(rect.top - mDelSelectedItemBmp.getHeight()/2));
